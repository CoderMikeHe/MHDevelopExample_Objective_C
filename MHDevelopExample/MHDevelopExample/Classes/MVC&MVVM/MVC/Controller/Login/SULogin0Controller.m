...
@implementation SULogin0Controller
...
- (IBAction)_loginBtnDidClicked:(UIButton *)sender {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [[NSUserDefaults standardUserDefaults] setValue:self.inputView.phoneTextField.text forKey:SULoginPhoneKey];
        [[NSUserDefaults standardUserDefaults] setValue:self.inputView.verifyTextField.text forKey:SULoginVerifyCodeKey];
        [[NSUserDefaults standardUserDefaults] synchronize];
        
        /// 保存用户数据 这个逻辑就不要我来实现了吧 假数据参照 [AppDelegate sharedDelegate].account
        
        /// 跳转主界面
        SUGoods0Controller *goodsVc = [[SUGoods0Controller alloc] initWithStyle:UITableViewStyleGrouped];
...
- (void)_setupSubViews
{
    ...
    /// 从沙盒中取出数据
    inputView.phoneTextField.text = [[NSUserDefaults standardUserDefaults] objectForKey:SULoginPhoneKey];
    inputView.verifyTextField.text = [[NSUserDefaults standardUserDefaults] objectForKey:SULoginVerifyCodeKey];
    /// 验证登录按钮的有效性
    [self _textFieldValueDidChanged:nil];
