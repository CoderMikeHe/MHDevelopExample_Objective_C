...
static void *CMHObjectCachedPropertyKeysKey = &CMHObjectCachedPropertyKeysKey;
...
// occurred.
static BOOL CMHValidateAndSetValue(id obj, NSString *key, id value, BOOL forceUpdate, NSError **error) {
...
+ (NSSet *)propertyKeys {
    NSSet *cachedKeys = objc_getAssociatedObject(self, CMHObjectCachedPropertyKeysKey);
    ...
    objc_setAssociatedObject(self, CMHObjectCachedPropertyKeysKey, keys, OBJC_ASSOCIATION_COPY);
...
+ (void)enumeratePropertiesUsingBlock:(void (^)(objc_property_t property, BOOL *stop))block {
    ...
    while (!stop && ![cls isEqual:CMHObject.class]) {
        unsigned count = 0;
...
- (BOOL)validate:(NSError **)error {
    for (NSString *key in self.class.propertyKeys) {
        ...
        BOOL success = CMHValidateAndSetValue(self, key, value, NO, error);
